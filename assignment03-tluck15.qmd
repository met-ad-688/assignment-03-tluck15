...
```{python}
df2 = df.withColumn("counter", F.lit(1))
```
...
```{python}
median_salaries2["LOT_V6_SPECIALIZED_OCCUPATION_NAME"] = pd.Categorical(
    median_salaries2["LOT_V6_SPECIALIZED_OCCUPATION_NAME"],
    categories=sorted_employment_types2,
    ordered=True
)
```
